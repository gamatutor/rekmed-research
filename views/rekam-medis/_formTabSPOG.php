                    ...
                    <div class="tab-pane fade active in" id="tab_1">
                        ...
                        <div class="row">
                            ...
                            if(!$model->isNewRecord)
                                $model->spog_hpth = date('d-m-Y', strtotime($model->spog_hpth)); ?>
                            <div class="col-md-3 col-sm-6"><?= $form->field($model, 'spog_hpth')->widget(\yii\widgets\MaskedInput::className(), [
                                'mask' => '99-99-9999',
                                'options' => [
                                    'placeholder' => 'dd-mm-yyyy',
                                    'class' => 'form-control hpth',
                                ]
                            ]) ?></div>
                            <div class="col-md-3 col-sm-6"><?= $form->field($model, 'HPL')->textInput(['class'=>'HPL form-control']) ?></div>
                            <div class="col-md-3 col-sm-6"><?= $form->field($model, 'mensHariKe')->textInput(['class'=>'mensHariKe form-control']) ?></div>
                            <div class="col-md-3 col-sm-6"><?= $form->field($model, 'usiaKehamilan')->textInput(['class'=>'usiaKehamilan form-control']) ?></div>
                        </div>
...
<?php $this->registerjs("
    ...
    function hitungSPOGAttr()
    {
        var from = $('.hpth').val().split('-');
        var hpth = new Date(from[2], from[1] - 1, from[0]);
        if (hpth!='Invalid Date')
        {
            var hpl = addDays(hpth, 280);

            var mensHariKe = parseInt((new Date() - hpth) / (1000 * 60 * 60 * 24))+1;
            var usiaKehamilan = (mensHariKe-1)/7;
            
            $('.HPL').val(formatDate(hpl));
            $('.mensHariKe').val(mensHariKe);
            $('.usiaKehamilan').val(Math.floor(usiaKehamilan));
        }
    }
